...
class NN:
    ...
    def retropropagation(self,x,differences,pas,storage):
        ...
        if storage==False:
            for i in range(0,self.sizeHiddenLayer2):
                for j in range(0,self.sizeOutput):
                    self.LastLayerEntryWeights[j,i]+=self.LastLayerStorage[j,i]
                    self.LastLayerStorage[j,i]=0
                    
            for i in range(0,self.sizeHiddenLayer1):
                for j in range(0,self.sizeHiddenLayer2):
                    self.HiddenLayer2EntryWeights[j,i]+=self.HiddenLayer2Storage[j,i]
                    self.HiddenLayer2Storage[j,i]=0
                    
            for i in range(0,self.sizeHiddenLayer1):
                for j in range(0,self.sizeInput):
                    self.HiddenLayer1EntryWeights[i,j]+=self.HiddenLayer1Storage[i,j]
...
#changer std par logstd
        ...
        -(actionPrime[0]-mu1Prime)**2*math.exp(-2*lsigma1Prime)/2-(actionPrime[1]-mu2Prime)**2*math.exp(-2*lsigma2Prime)/2
        ...
        if argmin==0:
            ...
            differences[0]=differences[0]+Q1NN.DerivativesLastLayer[8]- \
                alpha2*((-actionTilde[0]+mu1Tilde)*math.exp(-2*lsigma1Tilde)) 
            differences[1]=differences[1]+Q1NN.DerivativesLastLayer[9]- \
                alpha2*((-actionTilde[1]+mu2Tilde)*math.exp(-2*lsigma2Tilde)) 
            differences[2]=differences[2]+Q1NN.DerivativesLastLayer[8]*gauss1*math.exp(lsigma1Tilde)- \
                alpha2*((-actionTilde[0]+mu1Tilde)*math.exp(-2*lsigma1Tilde))*gauss1*math.exp(lsigma1Tilde)
            differences[3]=differences[3]+Q1NN.DerivativesLastLayer[9]*gauss2*math.exp(lsigma2Tilde)- \
                alpha2*((-actionTilde[1]+mu2Tilde)*math.exp(-2*lsigma2Tilde))*gauss2*math.exp(lsigma2Tilde)
        else:
            Q2NN.output(stateAction)
            Q2NN.derivatives()

            differences[0]=differences[0]+Q2NN.DerivativesLastLayer[8]- \
                alpha2*((-actionTilde[0]+mu1Tilde)*math.exp(-2*lsigma1Tilde)) 
            differences[1]=differences[1]+Q2NN.DerivativesLastLayer[9]- \
                alpha2*((-actionTilde[1]+mu2Tilde)*math.exp(-2*lsigma2Tilde)) 
            differences[2]=differences[2]+Q2NN.DerivativesLastLayer[8]*gauss1*math.exp(lsigma1Tilde)- \
                alpha2*((-actionTilde[0]+mu1Tilde)*math.exp(-2*lsigma1Tilde))*gauss1*math.exp(lsigma1Tilde)
            differences[3]=differences[3]+Q2NN.DerivativesLastLayer[9]*gauss2*math.exp(lsigma2Tilde)- \
                alpha2*((-actionTilde[1]+mu2Tilde)*math.exp(-2*lsigma2Tilde))*gauss2*math.exp(lsigma2Tilde)                                                    
